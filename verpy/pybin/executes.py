...
#        Root = get_param(wrds,'root')    
    ...
    if (wrds[0]=='list_instances'):
        ...
        res.sort()
        res.reverse()
        Pattern = string.replace('%5d  %WWWs    %s','WWW',str(Max+3))
        Tot=0
        for Len,Type in res:
    ...
    log_screen('knowns: %s'%string.join(RegisteredList,' '))
...
def importing(Fname,Env,Original=True):
    ...
    if (len(Fname)>2)and(Fname[-3:]=='.py'):
        Fname = Fname[:-3]
    if ('/' in Fname):
        wrds2 = string.split(Fname,'/')
        Fname = wrds2[-1]
        Path = string.join(wrds2[:-1],'/')
