...
def help_main(Env,Color=False,Volt=False):
    ...
    for Module in Env.Modules:
        ...
        if '-clean' in Env.params: cleanParameters(Mod)
        Sigs0,Regs0 = dump_instance(Fout,Env,Mod,'-simple' in Env.params,'-lower' in Env.params,Color,Volt)
        Sigs.extend(Sigs0)
        Regs.extend(Regs0)
    for Module in Env.Modules:
        Mod = Env.Modules[Module]
        dump_dut(Fout,Module,Sigs,Regs,Mod)
    dump_footer(Fout,Sigs,Regs,False)
#    dump_empty_module(Mod)
    if '-tb' in Env.params: prepare_tb(Env)
...
def prepare_tb(Env):
    ...
    Fcomp.write('iverilog  -o tb.vvp -g2012 \\\n')
    Fcomp.write('    tb.v \\\n')
...
def dump_instance(Fout,Env,Mod,Simple=False,allPinsLowerCase = False,Color = False,Volt = False):
...
def dump_footer(Fout,Sigs,Regs,Simple=False):
    if 'clk' not in Sigs:
        Fout.write('reg clk;\n')
...
def dump_dut(Fout,Name,Sigs,Regs,Mod,allPinsLowerCase=False,Color=False):
    Fout.write('%s dut (\n'%(Name))
    Pref=' '
    for Sig in Sigs:
        DirHL = Mod.nets[Sig]
