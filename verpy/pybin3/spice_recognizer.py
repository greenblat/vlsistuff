...
LOGICALS = ('and or nand nor inv xor xnor').split()
def help_main(Env):
    Mod = Env.Current
    writeOut(Mod,'stp0')
    ...
    while (Ok>0):
        Ok = scanDesignForPattern(Mod)
    recognizePattern2(Cmos_and_pattern,Mod)
    removeOrphans(Mod)
    writeOut(Mod,'stp2')
    gateMerges(Mod)
    writeOut(Mod,'stp3')
...
Cmos_and_pattern = ((
    'cmos i=?II o=?OO gn=?GN gp=?GP'
    ,'nmos g=?GP d=?DD s=?SS b=vss'
    ,'! anyone vss ?DD ?SS'
),cmosAnd)
XOR_pattern = ((
