...
integer  Ids;   initial Ids=0;
integer  clockids;   initial clockids=0;
reg [1023:0] Flops [0:4196];
reg [1023:0] Clocks [0:255];
wire [63:0] clk;
integer CYCLE = 20;

reg clk_enable; initial clk_enable=0;
clockGenerator clkgen (.CLK(clk),.period(CYCLE),.enable(clk_enable));

'''


def dump_instance(Mod):
    Name = Mod.Module
    ...
    for Sig in Sigs:
        ...
        if (Sig == 'clk'):
            ...
            Fout.write('wire [63:0] %s;\n'%(Sig))
            Fout.write('reg %sx;initial %sx=0;\n'%(Sig,Sig))
            Fout.write('inputGenerator %s_inp (.din(%sx),.DIN(%s));\n' % (Sig,Sig,Sig))
            Regs += '    %sx = 0;\n' % Sig
        elif ('output' in Dir1):
            Fout.write('wire [63:0] %s;\n'%(Sig))
        elif ('inout' in Dir1):
