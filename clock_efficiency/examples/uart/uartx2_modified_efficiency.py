...
def subbit(Sig):
    ...
    return Bus,Ind
class uartx2_modified_efficiency(logs.driverClass):
    ...
    def onFinish(self):
        logs.log_info('ONFINISH of clock efficiency in uartx2_modified') 
        Used,Wasted,Dropped = 0,0,0
        Keys = list(self.Counters.keys())
        Keys.sort()
        for Sig in Keys:
            logs.log_info('uartx2_modified %40s          used=%8d   wasted=%8d   dropped=%8d   width=%d' % (Sig,self.Counters[Sig][0],self.Counters[Sig][1],self.Counters[Sig][2]-self.Counters[Sig][0],WIDTH[Sig]))
            Used += self.Counters[Sig][0] * WIDTH[Sig]
            Wasted += self.Counters[Sig][1] * WIDTH[Sig]
            Dropped += self.Counters[Sig][2] * WIDTH[Sig]
        logs.log_info('-----------------------------------------------------------------')
        logs.log_info('Total for uartx2_modified:      used=%d wasted=%d dropped=%d' % (Used,Wasted,Dropped-Used))
        logs.log_info('-----------------------------------------------------------------\
\
')
    ...
    def run_posedge_clk(self):
        ...
        Vld = self.evaluate_valid(['||', ['!', 'rst_n'], ['&&', ['question', 'rxd_filt', 'is_zero', 'is_one'], ['&&', 'rx_midbit', 'rst_n']]])
        if (Vld):
        ...
        if self.changed_input("rxd_filt",['is_one']):
        ...
        Vld = self.evaluate_valid(['||', ['!', 'rst_n'], ['&&', ['||', 'negedge_rxd', 'rx_active'], 'rst_n']])
        if (Vld):
        ...
        Vld = self.evaluate_valid(['||', ['!', 'rst_n'], ['&&', 'tx_start', 'rst_n'], ['&&', 'tx_shift', ['&&', ['!', 'tx_start'], 'rst_n']], ['&&', ['&&', ['!', 'tx_active'], ['!=', 'txsr', ['hex', '11', '7ff']]], ['&&', ['!', 'tx_shift'], ['&&', ['!', 'tx_start'], 'rst_n']]]])
        if (Vld):
