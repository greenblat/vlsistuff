...
def work_on_lines(lines,filename):
    global pre_defines
    global Tempindex
    wrds = string.split(filename,'/')
    wrds = wrds[:-1]
    directory = string.join(wrds,'/')
    ifdefstate = [1]
    total = len(lines)
    sofar=0
    guard=0
        ...
        if (len(words)>0):
            if (words[0]=='`include'):
                ...
                if (ok(ifdefstate)):
                    ...
                    for lll in mlines:
                        wordsx = string.split(lll)
                        if (wordsx[0]=='`define'):
                            if (len(wordsx)>=3):
                                value1 = string.join(words[2:],' ')
                                wrds2 = string.split(value1,'//')
                                value = wrds2[0]
    ...
    for line1 in wholelib:
        ...
        if (state==0):
            if (len(wrds)>0)and(wrds[0]=='module'):
                ...
                if (Module[-1]==';'):
                    Module = Module[:-1]
                Mudule  = normalizeName(Module)
                File=open(dir+'/'+Mudule+'.v','w')
...
#                       File.write('`define %s %s\n'%(A,B))
            ...
            if (len(wrds)>0)and(has_end(wrds[0])):
                ...
                File.write(string.replace(line,Module,Mudule))
        elif (state==10):
            if (len(wrds)>0)and(has_end(wrds[0])):
                File.write(line)
                File.close()
