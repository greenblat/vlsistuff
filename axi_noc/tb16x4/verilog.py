...
merger = [0,0,0,0,0]
merger[0] = mergeAluka.mergeAlukaClass('tb.dut.merger0',Auxs,'','AlukaM0')
merger[1] = mergeAluka.mergeAlukaClass('tb.dut.merger1',Auxs,'','AlukaM1')
merger[2] = mergeAluka.mergeAlukaClass('tb.dut.merger2',Auxs,'','AlukaM2')
merger[3] = mergeAluka.mergeAlukaClass('tb.dut.merger3',Auxs,'','AlukaM3')
merger[4] = mergeAluka.mergeAlukaClass('tb.dut.merger4',Auxs,'','AlukaM4')
...
class driverMonitor(logs.driverClass):
    ...
    def wr_run(self):
        ...
        for X in range(Len):
            ...
            Ref.append(self.Data0)
            self.Data0 += 1

        logs.log_info('GEN %s addr=%x len=%x datas=%s' % (Mst,Addr,Len,Ref),'drv')
