...
# style="stroke:rgb(255,0,0);stroke-width:2"
    def aline(self,X0,Y0,X1,Y1,Color=False):
        ...
        Y1 = self.prty(Y1)
        if self.Dash[1]==0:
            self.File.write('<line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="2" />\n'%(X0,Y0,X1,Y1,self.prtColor()))
        else:
            self.File.write('<line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="2" stroke-dasharray="%d,%d" />\n'%(X0,Y0,X1,Y1,self.prtColor(),int(self.Dash[0]*self.Scale),int(self.Dash[1]*self.Scale)))
...
#        self.File.write('[%s %s] 0 setdash\n'%(A,B))
    ...
    def prtColor(self):
        Str = 'rgb(%d,%d,%d)'%(int(255*self.RGB[0]),int(255*self.RGB[1]),int(255*self.RGB[2]))
        return Str
