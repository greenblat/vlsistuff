...
from oglGeoms import screenShot
import helpString
...
def init():
    ...
    set_context('width',640)
    set_context('height',480)
...
from renders import render_line, render_aline,render_circle,render_text,compute_matrix,translate_mag,translate_rotate,translate_move,matrix_mult,render_fcircle,render_arc,world_coord_fl,render_frect,render_rect
    ...
    def draw(self,Matrix,Color='none'):
        ...
        for (Which,List) in self.geoms:
            if (Which=='circle'):
                ...
                log_error('missing ogl pic geom %s in %s'%(Which,self.Picture))
...
class DetailClass:
    ...
    def get_matrix(self):
        ...
        self.matrix = [Scale,0,-X0*Scale,0,Scale,-Yoff]
        return self.matrix
...
#                Glbs.details[Root].params[Key].move(PP[0]-WasX,PP[1]-WasY)
        ...
        log_error('dont know to move "%s"'%What)
...
def use_keystroke(Uni,X,Y):
    ...
    if (Uni=='t'):
    ...
    elif ((len(str(Uni))==1)and(str(Uni) in '123456789'))or(Uni in ['Z','z']):
        if (Uni=='Z'):
            Uni=2
        elif (Uni=='z'):
            Uni=7
    ...
    elif (Uni in [ K_RIGHT,K_LEFT,K_UP,K_DOWN]):
        if (Uni==K_RIGHT):
            Dx,Dy=0.1,0
        elif (Uni==K_LEFT):
            Dx,Dy=-0.1,0
        ...
        if (Who):
            ...
            if (Who=='instance'):
                ...
                log_error('dont know to move who="%s" inst="%s"'%(Who,Inst))
    elif (Uni=='c'):
        (Who,Inst)= select_object((X,Y))
        if (Who=='instance'):
        ...
        if (Who):
            if (Who=='instance'):
                ...
                Glbs.set_context('banner','%s: dont know to move who="%s" %s'%(Root,Who,Inst))
                log_info('dont know to move "%s %s"'%(Who,Inst))
    elif (Uni=='d'):
        if 'grouping' in Glbs.contexts:
            [_,_,List]=get_context('grouping')
