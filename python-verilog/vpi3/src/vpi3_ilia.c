...
// this is version that works on my MAC with python3.14
// lines 1512-1535 setup the 3.14 python version.
...
veri_peek(PyObject *self,PyObject *args) {
    ...
    if (strlen(pathstring)<=5) {
        printf("ERROR! PEEK for too short name (<=5 chars)  ||%s||\n",pathstring);
        return Py_BuildValue("s", "XXXXXXXXXXXXXXXXXXXXXXXXXX");
    }
...
static void dump_config(const PyConfig *config)
{
    wprintf(L"home = %ls\n", config->home ? config->home : L"(null)");
    wprintf(L"program_name = %ls\n",
            config->program_name ? config->program_name : L"(null)");
    wprintf(L"executable = %ls\n",
            config->executable ? config->executable : L"(null)");
    wprintf(L"prefix = %ls\n",
            config->prefix ? config->prefix : L"(null)");
    wprintf(L"exec_prefix = %ls\n",
            config->exec_prefix ? config->exec_prefix : L"(null)");
    wprintf(L"base_prefix = %ls\n",
            config->base_prefix ? config->base_prefix : L"(null)");
    wprintf(L"base_exec_prefix = %ls\n",
            config->base_exec_prefix ? config->base_exec_prefix : L"(null)");

    wprintf(L"module_search_paths:\n");
...
//     dlopen("libpython3.10.so",RTLD_LAZY | RTLD_GLOBAL);
    ...
    dlopen("libpython3.14.dylib",RTLD_LAZY | RTLD_GLOBAL);
    dlopen("libpython3.14.so",RTLD_LAZY | RTLD_GLOBAL);
    PyImport_AppendInittab("veri", PyInit_veri);

    PyConfig_InitIsolatedConfig(&config);

    config.module_search_paths_set = 1;
    check(PyWideStringList_Append(&config.module_search_paths,
                                  L"/opt/homebrew/Cellar/python@3.14/3.14.4_1/Frameworks/Python.framework/Versions/3.14/lib"));
    check(PyWideStringList_Append(&config.module_search_paths,
                                  L"/opt/homebrew/Cellar/python@3.14/3.14.4_1/Frameworks/Python.framework/Versions/3.14/lib/python3.14"));
    check(PyWideStringList_Append(&config.module_search_paths,
                                  L"/opt/homebrew/Cellar/python@3.14/3.14.4_1/Frameworks/Python.framework/Versions/3.14/lib/python3.14/lib-dynload/"));

    wprintf(L"Configured module_search_paths:\n");
    for (Py_ssize_t i = 0; i < config.module_search_paths.length; i++) {
        wprintf(L"  %ls\n", config.module_search_paths.items[i]);
    }
...
//    PyConfig_InitPythonConfig(&config);
    ...
    printf("init python\n");
    printf(L"PythonHome: %ls\n", Py_GetPythonHome());
    wprintf(L"PythonPath: %ls\n", Py_GetPath());
    wprintf("init python\n");
